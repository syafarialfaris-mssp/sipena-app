...
app.put('/api/system', async (req, res) => {
    ...
    await System.findOneAndUpdate({}, dataUpdate, { upsert: true, new: true });
...
app.post('/api/login-ortu', async (req, res) => {
    const { nisn, no_hp } = req.body;
    const siswa = await Siswa.findOne({ nisn, no_hp_ortu: no_hp }).lean();
    if (siswa) res.json({ success: true, data: { ...siswa, role: "ortu", id: siswa._id } });
    else res.json({ success: false, message: "NISN atau No HP Orangtua Salah!" });
});

app.put('/api/user/avatar', async (req, res) => {
    const { role, identifier, avatar } = req.body;
    const avatarUrl = await uploadToCloudinary(avatar, 'avatars'); 
...
app.get('/api/guru', async (req, res) => {
    const gurus = await Pegawai.find({ role: 'guru' }); res.json({ success: true, data: gurus });
});
app.post('/api/guru', async (req, res) => {
    await Pegawai.create({ ...req.body, role: "guru", avatar: "" }); res.json({ success: true, message: "Guru berhasil ditambahkan!" });
});
app.post('/api/import-guru', async (req, res) => {
    ...
    for (let row of dataArray) {
        ...
        if(guruExist) {
            const cekDuplikat = guruExist.mengajar.find(m => m.kelas == row.kelas && m.mapel == row.mapel);
            if(!cekDuplikat) { guruExist.mengajar.push({ kelas: String(row.kelas), mapel: String(row.mapel) }); await guruExist.save(); }
        } else {
            await Pegawai.create({
                nama: row.nama, email: row.email, password: String(row.password || "123"), role: "guru", avatar: "",
                mengajar: [{ kelas: String(row.kelas), mapel: String(row.mapel) }]
            });
        }
    }
...
app.put('/api/guru/:id', async (req, res) => {
    ...
    if (up) res.json({ success: true, message: "Data Guru diupdate!" }); else res.json({ success: false, message: "Guru tidak ditemukan" });
});
app.delete('/api/guru/:id', async (req, res) => {
    await Pegawai.findByIdAndDelete(req.params.id); res.json({ success: true, message: "Data Guru dihapus!" });
});

app.get('/api/siswa', async (req, res) => {
    const siswas = await Siswa.find(); res.json({ success: true, data: siswas });
});
app.post('/api/siswa', async (req, res) => {
    await Siswa.create({...req.body, avatar: ""}); res.json({ success: true, message: "Murid ditambahkan!" });
});
app.post('/api/import-siswa', async (req, res) => {
    const importData = req.body.map(s => ({...s, avatar: ""}));
    try { await Siswa.insertMany(importData, { ordered: false }); } catch(e) {}
    res.json({ success: true, message: "Data murid diimport!" });
});
app.put('/api/siswa/:nisn', async (req, res) => {
    const up = await Siswa.findOneAndUpdate({ nisn: req.params.nisn }, req.body);
    if (up) res.json({ success: true, message: "Data Murid diupdate!" }); else res.json({ success: false, message: "Murid tidak ditemukan" });
});
app.delete('/api/siswa/:nisn', async (req, res) => {
    await Siswa.findOneAndDelete({ nisn: req.params.nisn });
    await Nilai.deleteMany({ nisn: req.params.nisn });
    res.json({ success: true, message: "Data Murid dihapus permanen!" });
});
...
app.post('/api/input-nilai-bulk', async (req, res) => {
    ...
    for (let item of dataArray) {
        ...
        const dataBaru = {
            ...
            dilihat_ortu_rem1: false, dilihat_ortu_rem2: false, dilihat_ortu_rem3: false,
            ...item, lampiran_file: urlLampiran 
        };
        if (dataBaru.kktp) dataBaru.status_tuntas = Number(dataBaru.skor) >= Number(dataBaru.kktp) ? "TUNTAS" : "TIDAK TUNTAS";
        payload.push(dataBaru);
    }
    await Nilai.insertMany(payload);
    res.json({ success: true, message: `Berhasil menyimpan ${dataArray.length} data nilai!` });
});

app.get('/api/nilai', async (req, res) => {
    const nilais = await Nilai.find(); res.json({ success: true, data: nilais });
});
app.get('/api/nilai/:nisn', async (req, res) => {
    const nilais = await Nilai.find({ nisn: req.params.nisn }); res.json({ success: true, data: nilais });
});

app.put('/api/nilai/:id/edit', async (req, res) => {
    const b = req.body;
    let n = await Nilai.findById(req.params.id);
    if(!n) return res.json({ success: false, message: "Data tidak ditemukan!" });

    if (b.skor !== undefined && n.skor !== Number(b.skor)) { n.skor = Number(b.skor); n.dilihat_ortu = false; }
    if (b.catatan !== undefined) n.catatan = b.catatan;
    if (b.lampiran_link !== undefined) n.lampiran_link = b.lampiran_link;

    if (b.lampiran_file !== undefined) {
        if (b.lampiran_file && b.lampiran_file.startsWith('data:')) n.lampiran_file = await uploadToCloudinary(b.lampiran_file, 'lampiran_tugas');
        else if (b.lampiran_file === "") n.lampiran_file = null; 
    }

    if (b.remedial_1 !== undefined) {
        let val = (b.remedial_1 === "" || b.remedial_1 === null) ? null : Number(b.remedial_1);
        if (n.remedial_1 !== val) { n.remedial_1 = val; n.dilihat_ortu_rem1 = false; }
    }
    if (b.catatan_rem1 !== undefined) n.catatan_rem1 = b.catatan_rem1;
    if (b.lampiran_link_rem1 !== undefined) n.lampiran_link_rem1 = b.lampiran_link_rem1;
    if (b.lampiran_file_rem1 !== undefined) {
        if (b.lampiran_file_rem1 && b.lampiran_file_rem1.startsWith('data:')) n.lampiran_file_rem1 = await uploadToCloudinary(b.lampiran_file_rem1, 'lampiran_tugas');
        else if (b.lampiran_file_rem1 === "") n.lampiran_file_rem1 = null;
    }

    if (b.remedial_2 !== undefined) {
        let val = (b.remedial_2 === "" || b.remedial_2 === null) ? null : Number(b.remedial_2);
        if (n.remedial_2 !== val) { n.remedial_2 = val; n.dilihat_ortu_rem2 = false; }
    }
    if (b.catatan_rem2 !== undefined) n.catatan_rem2 = b.catatan_rem2;
    if (b.lampiran_link_rem2 !== undefined) n.lampiran_link_rem2 = b.lampiran_link_rem2;
    if (b.lampiran_file_rem2 !== undefined) {
        if (b.lampiran_file_rem2 && b.lampiran_file_rem2.startsWith('data:')) n.lampiran_file_rem2 = await uploadToCloudinary(b.lampiran_file_rem2, 'lampiran_tugas');
        else if (b.lampiran_file_rem2 === "") n.lampiran_file_rem2 = null;
    }

    if (b.remedial_3 !== undefined) {
        let val = (b.remedial_3 === "" || b.remedial_3 === null) ? null : Number(b.remedial_3);
        if (n.remedial_3 !== val) { n.remedial_3 = val; n.dilihat_ortu_rem3 = false; }
    }
    if (b.catatan_rem3 !== undefined) n.catatan_rem3 = b.catatan_rem3;
    if (b.lampiran_link_rem3 !== undefined) n.lampiran_link_rem3 = b.lampiran_link_rem3;
    if (b.lampiran_file_rem3 !== undefined) {
        if (b.lampiran_file_rem3 && b.lampiran_file_rem3.startsWith('data:')) n.lampiran_file_rem3 = await uploadToCloudinary(b.lampiran_file_rem3, 'lampiran_tugas');
        else if (b.lampiran_file_rem3 === "") n.lampiran_file_rem3 = null;
    }
    ...
    await n.save();
    res.json({ success: true, message: "Data Nilai berhasil diperbarui!" });
});

app.delete('/api/nilai/:id', async (req, res) => {
    await Nilai.findByIdAndDelete(req.params.id); res.json({ success: true, message: "Data Nilai berhasil dihapus!" });
});
...
app.put('/api/nilai/:id/feedback', async (req, res) => {
    ...
    const tahap = req.body.tahap || 'utama';
    ...
    if (tahap === 'rem3') { n.dilihat_ortu_rem3 = true; n.feedback_rem3 = req.body.feedback; n.waktu_dilihat_rem3 = timestamp; }
    else if (tahap === 'rem2') { n.dilihat_ortu_rem2 = true; n.feedback_rem2 = req.body.feedback; n.waktu_dilihat_rem2 = timestamp; }
    else if (tahap === 'rem1') { n.dilihat_ortu_rem1 = true; n.feedback_rem1 = req.body.feedback; n.waktu_dilihat_rem1 = timestamp; }
    else { n.dilihat_ortu = true; n.feedback_ortu = req.body.feedback; n.waktu_dilihat = timestamp; }

    n.guru_sudah_baca = false;
    await n.save();
